...
export class ActivityStats {
    ...
    get totalKills() {
        return this._members.reduce(
            (total, current) => total + current.stats.kills,
            0,
        )
    }
    ...
    get totalDeaths() {
        return this._members.reduce(
            (total, current) => total + current.stats.deaths,
            0,
        )
    }
    ...
    get totalCharactersUsed() {
        return this._members.reduce(
            (total, curr) => total + curr.characterIds.length,
            0,
        )
    }
    ...
    get mostUsedWeapon() {
        return this._members
            ...
            .reduce((mostKills, current) =>
                (current?.kills ?? 0) > (mostKills?.kills ?? 0)
                    ? current
                    : mostKills,
            )
    }
